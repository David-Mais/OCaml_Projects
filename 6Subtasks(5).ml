...
let rec element_exists e l = 
  match l with
  |[] -> false
  |hd::tl -> if(hd = e) then true else element_exists e tl;;
...
let drop_dupl_custom_llist l = 
  let rec helper acc hl =
    match hl () with
    |NilC -> fun () -> NilC
    |ConsC (a,b) -> if (element_exists a acc) then helper acc b
    else fun () -> ConsC (a, helper (a::acc) b)
  in helper [] l;;

let drop_dupl_ocaml_llist l = 
  let rec helper acc hl = 
    match Lazy.force hl with
    |NilO -> lazy NilO
    |ConsO (a,b) -> if (element_exists a acc) then helper acc b
    else lazy (ConsO (a, helper (a::acc) b))
  in helper [] l;;
